...
Перший hits більшість borrowers на сьогодні (5-year fixed ~5%, stress = 7%; 5-year variable ~6%, stress = 8%). Тільки якщо contract rate < 3.25% does benchmark 5.25% kick in (rare in 2026).
...
CMHC insurance required if down payment < 20%. CMHC premium = % of mortgage amount, added to monthly payment:
...
Banks see TDS spike if cards near limit. Pay down balance to <50% utilization at least 60 days before application. Credit score affects rate (each 50-point band typically 0.10-0.25% difference).
